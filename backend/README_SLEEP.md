...
- 实际部署所有功能
- 开发新的功能
- 现在需要解决的问题：1.每次切换页面或者提交请求后，界面不会自动刷新，需要手动刷新后才会改变 2.后端提交运动和饮食记录没有bug，但是前端没有真正显示实际数据，一直保持无数据的状态,请修改对应部分的实现

我打开的目录是一个健康监测网站的工程，其中，后端已经由我编写完成了账号，饮食，睡眠，运动的相关api，
前端大部分内容由ai生成，并不与我现在的后端完全对应，现在存在以下问题：
1.部分模块之间相互切换时，网页不会自动更新，需要刷新后更新
2.前端大量模块使用虚拟数据，并没有使用后端数据库中的数据，也有可能是后端没有成功把数据记录到数据库中
现在请你解决上面的问题
